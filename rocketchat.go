...
func (chat *Chat) Logout() error {
  ...
  log.Println("logoutResp", logoutResp)
...
func (chat Chat) GetIncomingMessage(
  ...
  go func() {
    ...
    for {
      ...
      log.Debugf("Current room: %v", current)
...
func (chat Chat) getChannelsHistory(
  ...
  log.Debugf(
    "getChannelsHistory, latest: %s, oldest: %s",
    latest,
    oldest,
  )
  req, err := http.NewRequest(
...
func (chat Chat) getIMHistory(
  ...
  log.Debugf(
    "getIMHistory, latest: %s, oldest: %s",
    latest,
    oldest,
  )
  req, err := http.NewRequest(
...
func (chat Chat) PostMessage(body map[string]string) error {
  jsonValue, err := json.Marshal(body)
  if err != nil {
    return err
  }
...
func debugResponse(body io.Reader) error {
  data, err := ioutil.ReadAll(body)
  if err != nil {
    return err
  }
  log.Debugln("data", string(data))
  return nil
}
